...
@pytest.mark.asyncio
async def test_research_coordinator_refreshes_then_skips_fresh_context(tmp_path: Path):
...
@pytest.mark.asyncio
async def test_research_coordinator_rejects_insufficient_history(tmp_path: Path):
    store = StrategyPerformanceStore(tmp_path / "perf.db")
