...
def test_gen3_split_is_chronological_and_non_overlapping():
    bars=_bars(1000); train,validation,test=split_train_validation_test(bars)
    assert (len(train),len(validation),len(test))==(600,200,200); assert train[-1].time<validation[0].time; assert validation[-1].time<test[0].time; assert train+validation+test==bars
