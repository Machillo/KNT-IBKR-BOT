...
| 4 | The runtime selector had a learning store (bonus/AVOID); replay has none. | B/D | Learning drifted inside the forward window. | **Eliminated for shadow-only.** Learning is frozen (`learning_enabled=False`) and `selector_bonus`/`learning_mode` are journaled. Since round 4 learning is OFF by default in every mode (paper included). |
...
| N13 | Instrument type: the runtime and shadow-only analyse only IBKR `stockType` COMMON/ADR/REIT (ETFs, ETNs and unknown types excluded before deciding); replay has no `stockType`. | B | Documented. Journal- and CSV-universe replays can include ETFs the runtime would exclude. Fix: record `stockType` in the funnel (or the point-in-time file) and filter the same way. |
| N14 | GTC protective children (round 4) count as pending exposure across days at runtime (`portfolio/state.py` counts every working order). Replay counts only unfilled entries. | D | Runtime more conservative: each held position uses about 3x its notional against gross and cash. This is the effective position cap until PR-D adds explicit heat and max-position limits. |
